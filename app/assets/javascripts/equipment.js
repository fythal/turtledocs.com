...
function box_select(value){
	...
	if (ident == 'eq'){
		...
		get_document_list(id);
	}
}

function get_document_list(model_id){
	equipment_name = $('#eq_box_selected').text();
	$('#docs_equipment').html($('#eq_box_selected').text());
	var id = model_id;
	if(!id){
		id = $('#md_box_selected').attr('value');
	}
	...
	$.ajax({
		...
		type: "GET",
        url: "/equipment/docs_container/" + id + "/" + equipment_name,
...
function get_model_list(equipment_select_id, model_select_id = 'NULL'){
	$.ajax({
        beforeSend: function(xhr) {xhr.setRequestHeader('X-CSRF-Tokclose_formen', $('meta[name="csrf-token"]').attr('content'))},
		type: "GET",
        url: "/model/view/" + equipment_select_id + "/" + model_select_id,
        success: function(html){
